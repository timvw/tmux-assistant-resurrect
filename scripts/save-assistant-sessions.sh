...
main() {
	...
	MATCHES=$(awk '
		...
		}
		{
			...
			proc_args[pid] = line
			# Build adjacency list: SUBSEP-joined child pids per parent
	...
	local US=$'\x1f'
	local state_cache_available=0
	# Associative arrays require bash 4.0+. macOS ships bash 3.2 by default.
	# On older bash, the cache is disabled — sessions fall through to per-file
	# extraction (same as the jq < 1.7 fallback).
	if [ "${BASH_VERSINFO[0]:-0}" -ge 4 ]; then
		...
		if echo '{}' | jq 'input_filename' >/dev/null 2>&1; then
			...
			if [ ${#state_files[@]} -gt 0 ]; then
				while IFS="$US" read -r pid sid model env; do
					STATE_CACHE["$pid"]="$sid$US$model$US$env"
				done < <(
					jq -r '[
		...
		log "bash < 4.0 detected; state file cache disabled (falling through to per-file reads)"
	fi

	# Process only matched panes (those with a detected tool)
	local count=0
	if [ -n "$MATCHES" ]; then
		# Tab-delimited fields from awk. Limitation: literal tabs in cwd or
		# process args would shift field parsing (same class of issue as the
		# pipe delimiter in tmux pane output — extremely rare in practice).
		while IFS=$'\t' read -r target tool cpid cargs cwd; do
			[ -z "$target" ] && continue

			local cached_sid="" cached_model="" cached_env="null"
			if [ "$state_cache_available" = 1 ]; then
				local cached="${STATE_CACHE[$cpid]:-}"
				if [ -n "$cached" ]; then
					cached_sid="${cached%%"$US"*}"
					local rest="${cached#*"$US"}"
					cached_model="${rest%%"$US"*}"
					cached_env="${rest#*"$US"}"
